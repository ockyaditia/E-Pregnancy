<?php
	...
	if ($penolong_lain_lain == "none") {
		header('location:petugas_data_persalinan_ubah.php?ubah='. $kd_persalinan .'&error=1');
	} else if ($cara_persalinan == "none") {
		header('location:petugas_data_persalinan_ubah.php?ubah='. $kd_persalinan .'&error=2');
	} else if ($keadaan_ibu == "none") {
		header('location:petugas_data_persalinan_ubah.php?ubah='. $kd_persalinan .'&error=3');
	} else {
	
		include("petugas_data_persalinan_kueri_update.php");
		
		if($query){
			header('location:petugas_data_persalinan_form.php?success_ubah=1');
		}
		else{
			die(mysql_error());
		}
